...
- (void)presentContentView:(NSView*)view withCompletionHandler:(dispatch_block_t)handler {
  ...
  let modalSize = view.frame.size;
  self.box.frame = NSMakeRect(round((bounds.size.width - modalSize.width) / 2), round((bounds.size.height - modalSize.height) / 2), modalSize.width, modalSize.height);
