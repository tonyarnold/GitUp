...
+ (NSURL *)repositoryURLContainingURL:(NSURL *)url error:(NSError *__autoreleasing *)error {
  ...
  if (result == GIT_OK) {
    let repoPath = [[NSString alloc] initWithBytes:repoBuffer.ptr length:repoBuffer.size encoding:NSUTF8StringEncoding];
    let repoURL = [NSURL fileURLWithPath:repoPath];
