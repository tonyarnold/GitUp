            ...
            if (status == noErr) {
              ...
              while (1) {
                ...
                if (messagePort) {
                  ...
                  NSError *archivingError;
                  NSData* sendData = [NSKeyedArchiver archivedDataWithRootObject:message requiringSecureCoding:YES error:&archivingError];
                  if (sendData == nil) {
                    fprintf(stderr, "Failed archiving data: %s\n", archivingError.localizedDescription.UTF8String);
                    break;
                  }
                  ...
                  if (status == kCFMessagePortSuccess) {
                    ...
                    NSDictionary* response = [NSKeyedUnarchiver unarchivedObjectOfClasses:[NSSet setWithObjects:NSDictionary.class, NSString.class, nil] fromData:(__bridge NSData*)returnData error:&unarchivingError];
                    if (response == nil) {
                      fprintf(stderr, "Failed unarchiving response: %s\n", unarchivingError.localizedDescription.UTF8String);
                      break;
                    }
